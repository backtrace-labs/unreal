...
bool SSauceReportChecklist::IsIncludeCrashReporterEnabled()
{
	...
	if (const FConfigSection* Section = ConfigFile.Find(TEXT("/Script/UnrealEd.ProjectPackagingSettings")))
	{
...
FReply SSauceReportChecklist::OnRefreshClicked()
{
	...
	if (UProjectPackagingSettings* PackagingSettings = GetMutableDefault<UProjectPackagingSettings>())
	{
		...
		// Force GConfig to reload the Game config hierarchy:
		FString ConfigFilename;
		FConfigCacheIni::LoadGlobalIniFile(ConfigFilename, TEXT("Game"), nullptr, true);
	...
	return SNew(SVerticalBox)
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("HeaderTitle", "Sauce Report Configuration Checklist"))
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("HeaderSubtitle", "Verify your crash reporting configuration"))
